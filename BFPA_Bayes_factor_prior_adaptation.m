...
%
% Standalone Gaussian-Student's t Mixture (GSTM) variational-Bayes
% measurement update with Bayes factor prior adaptation (BFPA).
%
% Model:
%     p(z_kj | x_k, pi_kj) =
...
%
% Name-value options:
%     'NuMeasurement'     - Student-t degree of freedom, default 5.
%     'MaxIterations'     - Maximum VB iterations, default 100.
%     'Tolerance'         - Relative convergence tolerance, default 1e-6.
%     'PriorStrength'     - Beta prior strength, default 1.
%     'LambdaBounds'      - Bounds for E_lambda, default [0.05 1].
%     'Regularization'    - Numerical diagonal loading, default 1e-10.
%     'Verbose'           - Print iteration diagnostics, default false.
%
% Outputs:
...
%
% BFPA prior and GSTM update:
%     u_kj = r_kj/sqrt(S_kj)
%     BF_kj = p(u_kj | t_kj=1) / p(u_kj | t_kj=0)
%     a0_kj = BF_kj/(BF_kj+1), b0_kj = 1/(BF_kj+1)
%     kappa_kj = E_t_kj + (1 - E_t_kj)*E_lambda_kj
%     R_eff = diag(R_kj./kappa_kj)
% Large abnormal residuals reduce kappa_kj and increase equivalent noise.
