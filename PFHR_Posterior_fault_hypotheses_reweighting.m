...
%
% Name-value option:
%     'NoFaultPLCoefficient' - No-fault PL coefficient, default 2.
%
% Outputs:
...
%
% Formula:
%     M = union_{j=1}^{N_s} H_j,  P(M) = sum_j P(H_j)
%     eta_j = r_j' * inv(S_j) * r_j
%     p(z_k^(j)|H_j,z_{1:k-1}) =
%         [(2*pi)^m*|S_j|]^(-1/2) * exp(-eta_j/2)
%
%     alpha_j = P(H_j|z_k,M)
%             = P(H_j)*L_j / sum_r P(H_r)*L_r
%     pbar_j = P(M)*alpha_j
%     sum_j pbar_j = P(M)
